...
generate-app-link:
	echo "Gerando link e atualizando dados do aplicativo"
	@docker compose -f ${DOCKER_COMPOSE_FILE} exec php-fpm php artisan generate-apk-link
...
test: ## Roda o phpunit
ifdef group
	@docker compose -f ${DOCKER_COMPOSE_FILE} exec php-fpm vendor/bin/phpunit --group=$(group)
else ifdef filter
	@docker compose -f ${DOCKER_COMPOSE_FILE} exec php-fpm vendor/bin/phpunit --filter=$(filter)
else
	@docker compose -f ${DOCKER_COMPOSE_FILE} exec php-fpm vendor/bin/phpunit
endif
